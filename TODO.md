...
17. Verify scanner capture and routing:
   ...
   - During Closing, scan a new bundle whose Game ID has no saved ticket price. Assign a valid bin and confirm `Verify Game Price` opens before the unmatched item is cleared. Enter a price whose derived range excludes the scanned ticket and confirm the dialog remains open with the exact derived range. Save a fitting price and confirm the candidate placement becomes valid Closing evidence without creating a normal sale or activation. Cancel and confirm the bundle remains unmatched and finalization stays blocked.
