...
6. Verify ticket backfill:
   ...
   - Initial import collects bin + bundle/ticket placements first, then `Continue to Login` requires ticket price and bundle price once per distinct unconfigured Game ID. Repeated bundles for the same Game ID reuse that saved configuration; cancel, invalid input, game-save failure, or setup-save failure keeps initial import open.
   - After setup completes, regular scanning of any ticket serial in a new bundle, then bin, then missing game price/bundle price/name records activation gap-fill from the bundle's first ticket through the scanned ticket; for example `001` records `000-001`, quantity `2`, next `002`, and `008` records `000-008`, quantity `9`, next `009`.
   ...
   - If a placed bundle has no valid positive game price or bundle price, scanning a ticket must require complete game setup and must not record a `$0.00` sale.
   - Verify bundle-range completion: a `$20` game with an explicitly entered `$300` bundle and first ticket `000` permits `000`-`014`; scanning `014` records the final sale, keeps the bundle in the bin/Rdisplay as grey `Sold out`, and never creates ticket `015`. Verify a new Game ID has no default bundle price and cannot activate or finish receiving until both ticket price and bundle price are entered and persisted.
   - Verify premature sold-out recovery for every denomination: correcting a game's bundle price to extend its ticket range reopens affected grey bins at the first unclaimed ticket (using the ticket-claim ledger after restart), while genuinely complete bundles at the configured last ticket remain sold out. Later bundles with the same configured Game ID must reuse its saved ticket price and bundle price without prompting again.
...
10. Verify dedicated inventory receiving:
   ...
   - Updating inventory validates missing ticket and bundle prices and commits all staged bundles to unopened receiving inventory.
