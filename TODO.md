...
17. Verify scanner capture and routing:
   ...
   - Activate a new bundle so its provisional activation sale includes ticket `006`, then scan physically available ticket `006` during Closing. If the initial `006-006` reversal intersects a multi-ticket open-shift sale, confirm Closing expands through that sale's affected tail, preserves its valid prefix, and finalizes without a split-sale block.
