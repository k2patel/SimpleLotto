...
- A generated name such as `Game 1423` is a suggestion, not pre-entered text. Show it as the game-name placeholder, clear the suggestion while the operator edits that field, and use it as the saved fallback only when no name is entered.
- Required game-price entry parses positive whole dollars with an optional leading or trailing dollar sign, such as `20`, `$20`, or `20$`. Missing, malformed, or bundle-incompatible values must keep the dialog open and show a clear inline validation message; for example, `40$` is recognized as `$40` and then rejected because `$40` does not divide evenly into the automatic `$300` bundle total.
...
- Regular sold-out correction is immediate and transactional. Preserve prior sale rows, append auditable negative void rows and any required corrected positive replacement rows in the current open interval, rebuild the affected ticket claims, and update the bundle cursor in the same transaction. A zero-claim restored range is an inventory-only cursor correction. Prior closed-shift reports may be historically invalidated by this later correction; do not rewrite them.
...
- Settings > Scanner and Display must include Manager-only activation safety controls. `Require bundles to be received before activation` defaults on, and an optional comma-separated list of single starting digits limits previously unknown Game IDs. Clerks may see the saved policy but must not change it.
- When the received-inventory requirement is enabled, a previously unplaced bundle must exactly match Game ID + Bundle ID in Receiving before normal activation or Closing reconciliation. Reject a missing match before any bin or price dialog, audit it, and speak `Bundle invalid.` Existing active, sold-out, initial-import, and received bundles remain eligible for their normal workflows.
- Allowed Game ID starting digits apply only when admitting a previously unknown bundle identity. An empty list allows any starting digit. Reject a new identity with a disallowed starting digit before price/bin setup, audit it, and speak `Game invalid.` Receiving must apply the same admission check before adding a new bundle.
...
- Inside the dedicated Closing scan dialog, scanned bundles/tickets are inventory evidence, not normal live sales.
...
- A Closing scan is never included in its own forward gap sale because the scanned ticket is physically available. With stored current `004` and Closing scan `007`, record `004-006` sold and store `007` as current. If stored and scanned current are both `004`, record no sale and keep `004`.
...
- Closing bin cards must render from the latest effective bundle state, including temporary Closing evidence and any later completed usual operation, so a reconciled active bundle does not remain grey from its older persisted sold-out state.
...
- When an unmatched Closing scan introduces a bundle whose Game ID has no complete price setup, the existing bin-reconciliation dialog must dynamically add the bounded ticket-price field and automatic bundle total. Do not open a second price dialog for this missing-price case. Validate the bin and confirm that the scanned current ticket fits the range derived from the entered price before accepting the placement as Closing evidence. `Cancel` leaves the bundle unmatched and keeps finalization blocked. `Discard Scan` removes only that pending scan and its issue, retains all other accepted Closing evidence, and does not persist a game, placement, sale, activation, or ticket claim.
...
- When a closing scan does not fit the range derived from Game Information, show a red expandable correction bar on the Closing page. The bar must identify the game name, Game ID, Bundle ID, bin, scanned ticket, and configured range, and provide a direct `Fix Game Price` button. One click must present every outstanding price correction one by one until all are cleared or the operator cancels. The bounded `Verify Game Price` dialog shows only game name, Game ID, bin number, current ticket, editable whole-dollar ticket price, and the read-only automatic bundle total. With Manager access, save the corrected ticket price to Game Information and re-evaluate the same scan internally. Do not expose the scanned barcode or derived range calculation in the operator dialog. `Cancel`, insufficient role, or an invalid price keeps the correction bar available. `Discard Scan` removes only that rejected scan and issue while preserving other valid Closing evidence.
...
- Foregrounding a scanner-triggered dialog must preserve the current visible window state. A maximized window stays maximized, and a normal visible window keeps its size and position; restore only a minimized window, and show a hidden tray window without forcing a visible window into restored size.
...
- The supported non-ticket command labels are `BIN-<1-4 digits>` and `PRICE-<1-5 digits>`. Preserve the known-good `main` behavior that also accepts an exact bare 1-4 digit physical bin barcode such as `005`; do not obtain a bin by stripping characters or extracting a numeric suffix from arbitrary text. A price label payload is cents. Ticket scans must pass the configured state barcode parser.
...
- Undo/void creates one negative correction row in the current open interval that references the original sale ID. The original sale remains unchanged, and each original sale can have at most one correction. For a latest-tail Void Sale, the same transaction releases that sale's physical claims and restores the bundle cursor; backward-scan and Closing corrections continue to use their range-aware reconciliation rules.
- A physical ticket claim references the persistent sale ID that claimed it. Claims remain immutable during ordinary operation, except when an authorized transactional latest-tail void or range-aware reconciliation releases the exact restored ticket range under the ledger mutation guard.
