...
- When an unmatched Closing scan introduces a bundle whose Game ID has no complete price setup, assigning its physical bin creates only a candidate placement. Immediately require the bounded `Verify Game Price` dialog and confirm that the scanned current ticket fits the range derived from that price before accepting the placement as Closing evidence. Cancelling or entering a price that does not fit must leave the bundle unmatched and keep finalization blocked; do not persist a placement, sale, or activation.
